        ...
        {projects.map((data, index) => (
          ...
          >
            <div className="w-full p-4 flex flex-col font-mono gap-2 text-left">
              <h1 className="text-lg md:text-xl xl:text-2xl text-gray-800 dark:text-gray-200 tracking-wide">{data.name}</h1>
                <div className={`flex flex-wrap gap-4`}>
                  {data.svg.map((data, index) => 
                  <div key={index} className='text-gray-800 dark:text-gray-200 w-6 xl:w-8 h-6 xl:h-8'>
                    {data}
                  </div>
                    )}
                </div>
